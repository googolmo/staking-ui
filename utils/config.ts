...
export const VALID_NETWORKS: NetworkType[] = ["mainnet", "devnet", "testnet"];
...
export function getValidatorVoteAddress(): Address {
  const validatorVoteAddress = process.env.NEXT_PUBLIC_VALIDATOR_VOTE_ADDRESS;
  if (!validatorVoteAddress) {
    throw new Error("Validator Vote ENV is not set");
  }
  return address(validatorVoteAddress);
}
